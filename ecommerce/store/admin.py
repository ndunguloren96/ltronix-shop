...
from django.utils.html import format_html # Import format_html for safer HTML rendering
...
class ProductAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'price', 'category', 'stock', 'brand', 'sku', 'rating', 'reviews_count', 'image_preview')
    ...
    fields = (
        'name', 'description', 'price', 'digital',
        'category', 'stock', 'brand', 'sku', 'rating', 'reviews_count',
        'image_file', 'image_preview' # 'image_file' is for upload, 'image_preview' is for display
    )
    ...
    def image_preview(self, obj):
        if obj.image_file: # Check if image_file exists
            return format_html('<img src="{}" style="max-height: 100px; border-radius: 4px;" />', obj.image_file.url)
        return format_html('<span style="color: #888;">No Image</span>') # Fallback text
