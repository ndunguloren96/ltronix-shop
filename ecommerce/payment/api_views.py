        ...
        try:
            ...
            try:
                ...
                if transaction.is_callback_received:
                    ...
                    return Response({'detail': 'Callback already processed.'}, status=status.HTTP_200_OK)
                ...
                if result_code == 0:
                    ...
                    recipient_email = transaction.order.customer.email or (transaction.order.customer.user.email if transaction.order.customer.user else None)
                    if recipient_email:
                        send_payment_receipt_email(transaction.order, recipient_email)
                    logger.info(f"Payment receipt email sent to {recipient_email} for order {transaction.order.id}")
                else:
                    if result_code == 1032:
